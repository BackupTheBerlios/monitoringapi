...
int copy_packet(struct sk_buff *pskb,struct net_device *dev,struct packet_type *pt)
{
	u32 size,displ;
	mem_slot *slot;
...
struct packet_type packet_all_ops = { 
...
int init_module()
{
	...
	packet_all_ops.type = htons(ETH_P_ALL);
	dev_add_pack(&packet_all_ops);
...
void cleanup_module()
{
	...
	dev_remove_pack(&packet_all_ops);
