...
void handler()
{
	if(ioctl(sock,SIOCGCOOK_IP,&cis) || ioctl(sock,SIOCGCOOK_TCP,&cts))
	{
		perror("ioctl");
		exit(1);
	}
...
{
	if(ioctl(sock,SIOCSCOOK_IP,&cis) || ioctl(sock,SIOCSCOOK_TCP,&cts))
	{
		perror("ioctl");
		exit(1);
	}
...
int main(int argc, char **argv)
{
	...
	if((sock = socket(PF_MAPI,RTYPE,htons(ETH_P_ALL))) < 0)
	{
		perror("socket");
		exit(1);
	}
