...
void handler()
{
	if(ioctl(sock,SIOCGCOOK_IP,&cis) == -1)
	{
		perror("ioctl");
		exit(1);
	}
...
{
	if(ioctl(sock,SIOCSCOOK_IP,&cis) == -1)
	{
		perror("ioctl");
		exit(1);
	}
...
int main(int argc, char **argv)
{
	...
	if(bind_if_name(sock,IFNAME) || (rbuf_size = get_if_mtu(sock,IFNAME)) == -1 || if_promisc_on(sock,IFNAME))
	{
		perror("bind_if_name || get_if_mtu || if_promisc_on");
		exit(1);
	}
	...
	//rbuf_size += sizeof(struct ethhdr);
	rbuf_size -= sizeof(struct iphdr);
