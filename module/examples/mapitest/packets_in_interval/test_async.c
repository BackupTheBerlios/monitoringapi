...
void handler()
{
	...
	if(ioctl(sock,SIOCGPACKETS_IN_INTERVAL,&pin) == -1)
	{
		perror("ioctl");
		exit(1);
	}
...
void init_packets_in_interval_struct()
{
...
int main(int argc, char **argv)
{
	...
	init_packets_in_interval_struct();
	
	if(ioctl(sock,SIOCSPACKETS_IN_INTERVAL,&pin))
	{
		perror("ioctl");
		exit(1);
	}

	if(ioctl(sock,SIOCSASYNCPACKETS_IN_INTERVAL,&pin))
	{
		perror("ioctl");
		exit(1);
	}
