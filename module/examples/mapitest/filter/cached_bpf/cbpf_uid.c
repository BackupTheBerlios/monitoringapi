...
struct cached_bpf_filter_struct cached_bpf[MAX_FILTERS];
...
void handler()
{
	...
	{
		if(ioctl(sock,SIOCGCACHED_BPF_FILTER,&cached_bpf[i]))
		{
			perror("ioctl");
			exit(1);
		}
...
void apply_cached_bpf_filter(int sock)
{
	...
	for( i = 0 ; i < MAX_FILTERS ; i++)	
	{
		...
		memcpy(&(cached_bpf[i].fprog),&bpf_filter,sizeof(bpf_filter));
		
		if(ioctl(sock,SIOCSCACHED_BPF_FILTER,&cached_bpf[i]))
		{
			perror("ioctl");
			exit(1);
		}
